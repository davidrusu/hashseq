...
1. **Test vectors.** The derived constants (`TOMBSTONE`, `NewSeq`,
   `NewMap`), a worked node preimage per kind, an `object_id` derivation,
   and a small canonical snapshot — to be generated by the first
   implementation and locked in this file.
