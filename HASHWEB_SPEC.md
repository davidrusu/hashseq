...
- **Create**: an op whose payload/value is a creation artifact
  ...
  virtual node, never an op, generalizing origin unification (a standalone
  document's `doc_id` is the same class). Child ops anchor at and ref the
  origin id; the closure of an origin id is defined as `{X} ∪ closure(X)`,
  so the creation bridge still welds the document into one connected DAG,
